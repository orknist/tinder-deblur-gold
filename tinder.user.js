// ==UserScript==
// @name        Tinder Deblur Gold
// @namespace   Violentmonkey Scripts
// @match       https://tinder.com/*
// @grant       none
// @version     3.0
// @author      orknist
// @downloadURL https://raw.githubusercontent.com/orknist/tinder-deblur-gold/main/tinder.user.js
// @description Simple script using the official Tinder API to get clean photos of the users who liked you, + has Gold like button
// ==/UserScript==
...
async function unblur() {
	...
	for (let i = 0; i < teaserEls.length; ++i) {
		...
		if (teaserImage.includes('preview.gotinder.com')) {
			const userId = teaserImage.slice(29, 65);
			if (cache.has(userId)) continue;
			cache.add(userId, teaser);
			console.log(teaserImage);
			
			const infoContainer = teaserEl.parentNode?.lastElementChild;
			if (!infoContainer) {
				console.error(`Could not find info container`);
				continue;
			}
		...
		else if (teaserImage.includes('images-ssl.gotinder.com')) {
			const userId = teaserImage.slice(32, 56);
            console.log('teaserImage', teaserImage);
            const likedContentId = teaserImage.replace('.jpg','').slice(-36);

			if (cache.has(userId)) continue;

			try {
				if(!teaserGrandParentEl.classList.contains('my-gold-match-item')) {
					teaserGrandParentEl.classList.remove('Cur(p)');
					teaserGrandParentEl.classList.add('my-gold-match-item');

                    var new_element = teaserGrandParentEl.cloneNode(true);

                    const div = document.createElement("div");
                    div.classList.add('Pos(a)');
                    div.classList.add('Cur(p)');
                    div.style.top = 0;
                    div.style.right = 0;
                    ...
                    div.addEventListener('click', async function(){
                        ...
                        console.log('liked_content_id', likedContentId);
                        console.log('s_number', generateSNumber());
                        var response = await goldLikeProfile(userId, likedContentId, generateSNumber());
                        console.log(response);
                        alert(JSON.stringify(response));
                    });
                    new_element.appendChild(div);

					teaserGrandParentEl.parentNode.replaceChild(new_element, teaserGrandParentEl);					
                    teaserGrandParentEl = new_element;
				}
								...
								<div class="Pos(a) Fz($l) B(0) Trsdu($fast) Maw(80%) D(f) Fxd(c) like-user-name">
									<div class="D(f) Ai(c) Miw(0)">
										<div class="Ov(h) Ws(nw) As(b) Ell">
											<span class="Typs(display-3-strong)" itemprop="name">${user.name}</span>
										</div>
...
function generateSNumber() {
    var foo = '';
    for(i=0; i<16; ++i) foo += Math.floor(Math.random() * 10);
    return foo;
}

async function goldLikeProfile(profile_id, liked_content_id, s_number) {
	return fetch(`https://api.gotinder.com/like/${profile_id}?locale=en`, {
        method: "POST",
        headers: {
			'X-Auth-Token': localStorage.getItem('TinderWeb/APIToken') ?? '',
			platform: 'android',
		},
        body: JSON.stringify({s_number, liked_content_id, liked_content_type: 'photo'}),
	})
		.then((res) => res.json())
        .then((res) => res);
}
